...
def parse(opt_path, is_train=True):
    ...
    opt["path"]["root"] = osp.abspath(osp.join(__file__, osp.pardir))
...
def check_resume(opt, resume_iter):
    ...
    if opt["path"]["resume_state"]:
        if (opt["path"].get("pretrain_model", None) is not None):
            logger.warning("pretrain_model path will be ignored when resuming training.")

        opt["path"]["pretrain_model"] = osp.join(opt["path"]["models"], "{}_G.pth".format(resume_iter))
