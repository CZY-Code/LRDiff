...
class SDE(abc.ABC):
    ...
    def reverse_sde_step_mean(self, x, score, t): # x_t - \delta x
...
class IRSDE(SDE):
    ...
    def _initialize(self, max_sigma, T, schedule, eps=0.01):
        ...
        def get_sigma_bars(thetas_cumsum): #标准差v_t
            return torch.sqrt(max_sigma**2 * (1 - torch.exp(-2 * thetas_cumsum * self.dt)))
            
        if schedule == 'cosine':
            thetas = cosine_theta_schedule(T) #递增至1
    ...
    def reverse_sde(self, xt, T=-1, save_states=False, save_dir='sde_state', lowRanker=None):
        T = self.T if T < 0 else T #T=100
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1))): #遍历T步去噪
            score = self.score_fn(x, t)
            x = self.reverse_sde_step(x, score, t)
            if lowRanker is not None and t % 10 == 0:
                lowRankOut = lowRanker(x, x, [self.thetas[t]])
                lowRankOut = utils.tensor2img(lowRankOut.detach()[0].float().cpu()) #[0,1]->[0,255]
                x_out = utils.tensor2img(x.detach()[0].float().cpu())
                cv2.imshow(f'lowrank_t={t}', lowRankOut)
                cv2.imshow(f'Denoise_t={t}', x_out)
                cv2.waitKey(0)
